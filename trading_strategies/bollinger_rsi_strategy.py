...
class BollingerRSIStrategy(TradingStrategy):
    ...
    def process(self, row):
        ...
        if (
            current_row["close"] < current_row["BBL_20_2.0"]
            and current_row["RSI_14"] < self.rsi_oversold
            and (self.last_action == ACTION_SELL or self.last_action is None)
        ):
            return self.create_trade_action(ACTION_BUY, current_price)

        elif (
            current_row["close"] > current_row["BBU_20_2.0"]
            and current_row["RSI_14"] > self.rsi_overbought
